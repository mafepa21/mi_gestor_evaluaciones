...
- **SyncLan macOS arrancaba el SSE listener y /sync/pull antes de que el helper existiese**: en `KmpBridge.init` el bloque `#if os(macOS)` asignaba `pairedSyncHost = "127.0.0.1"` antes de lanzar el helper, provocando una cascada de `Connection refused` en `127.0.0.1:8765`. La corrección elimina esa asignación prematura: `pairedSyncHost` permanece `nil` hasta que `MacCommandCenterCoordinator` publica `syncHelperBecameReady` vía `NotificationCenter` cuando el helper reporta una IP LAN válida. `KmpBridge.setupObservers()` suscribe a esa notificación (macOS only) e invoca `notifyHelperReady(host:port:)` para arrancar listener y sync loop en el momento correcto. `notifyHelperStopped()` los detiene cuando el helper para.
