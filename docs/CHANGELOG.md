...
- `AppleAIOrchestrator` inicia agentes educativos internos (`Tutor`, `Evaluador`, `EF`) como router tipado sobre capacidades existentes, sin interfaz de chat ni nuevas fuentes de datos.
...
- La capa Apple IA agrupa capacidades especializadas por rol docente interno, manteniendo Foundation Models como generador de objetos renderizables y KMP como fuente de verdad.
...
- `git diff --check` y `plutil -lint kmp/iosApp/MiGestorKMPiOS.xcodeproj/project.pbxproj` completados correctamente tras iniciar el router de agentes educativos internos. `xcodebuild -list -project kmp/iosApp/MiGestorKMPiOS.xcodeproj` no pudo ejecutarse porque `xcode-select` apunta a `/Library/Developer/CommandLineTools`.
