...
- Instrumentos de evaluación: visualización de **Criterios de Evaluación LOMLOE (código y descripción literal de la ley, castellano y valenciano)** en el detalle y modales de evaluación (`RubricEvaluationView`, `RubricBulkEvaluationSheet`, `StructuredInstrumentEvaluationSheet` y `NotebookStudentInspector`). Nuevo catálogo oficial de Educación Física (`LomloeCriteriaCatalog.swift`, con los 17 criterios de 1º Bachillerato/ESO y su traducción oficial al valenciano) y nuevo componente reutilizable desplegable (`AssessmentCriteriaDisclosureView.swift`) que muestra los badges de criterios (ej. `CE 2.1`, `CE 3.2`) e integra una tarjeta colapsable con el texto literal oficial de la legislación educativa.
- Importación de SA (secuencia de sesiones): nuevo **formato semanal** (`sesiones_por_semana.docx`), donde la unidad de planificación es la SEMANA con un BLOQUE LARGO (90′) y un BLOQUE CORTO (30′), en vez de "SESIÓN N con versión simple y doble". Antes, un documento en este formato importaba **0 sesiones** ("No se han reconocido fichas de sesión"): el patrón de encabezado solo aceptaba `SESIÓN N`/`SESSION N`. Ahora se reconocen `SEMANA N`/`WEEK N` y, dentro de cada semana, los encabezados `BLOQUE LARGO`/`BLOQUE CORTO` (`LONG BLOCK`/`SHORT BLOCK`) con sus minutos declarados. Cada semana se importa como **dos sesiones**: el bloque largo (tablas horarias `Block 1` / `Break` / `Block 2`) y el bloque corto, cuya tabla única de dos variantes (`Time | Phase | Variante PREPARA | Variante CONSOLIDA`) se vuelca como dos secciones de desarrollo, una por variante, conservando franja y fase. La ficha `Item | Detail` de la semana (objetivo, `Criteria worked`, saberes, materiales y `Assessment`, etiqueta nueva) alimenta los dos bloques, igual que las preguntas guía, las variantes de dificultad y las adaptaciones, que son de la semana entera. Sustituye a la heurística `B5` de "versión simple y doble" (que tomaba 30′ y avisaba) para este formato: el bloque largo vale 90′ y el corto 30′, sin pérdida de contenido. Verificado sobre el documento piloto real (`SA 5 - Ultimate Frisbee`): 4 semanas → 8 bloques con objetivo, criterios (`CE 2.1`, `CE 3.2`…), material y desarrollo completos, y salida idéntica byte a byte en los 11 documentos del formato antiguo. **Limitación conocida**: la app todavía no ubica las sesiones automáticamente contra el horario del grupo, así que no puede decidir por sí sola qué bloque cae en el día de sesión doble; el import deja un aviso explicando que el bloque largo va en el día doble y el corto en el simple, y que el orden dentro de la semana cambia entre grupos. La colocación sigue siendo manual desde el Planner.
...
- Borrado total de datos: Corregido crash fatal `SIGABRT` / `vnode unlinked while in use` y crash de SwiftUI por falta de `EnvironmentObject` al ejecutar "Borrar todos los datos" desde Ajustes → Zona de riesgo.
  - Se anotan las 233 funciones `suspend` de la capa KMP con `@Throws(Throwable::class)` en `Contracts.kt` y `KmpContainer.kt` para asegurar la conversión de excepciones SQLite nativas a `NSError` manejables por Swift.
  - Se sustituye `VACUUM` por `PRAGMA wal_checkpoint(TRUNCATE)` en `DatabaseWipe.kt`, evitando desvincular ficheros `.db` en caliente mientras existan descriptores abiertos por otros procesos como el helper de Sync LAN.
  - Se añade sincronización activa del helper de Sync LAN en `SettingsDangerZoneView.swift` con retardo de 300ms antes del borrado SQL y guard de `needsRestart` en `MacCommandCenterCoordinator.swift`.
  - Se propaga `@EnvironmentObject private var bridge: KmpBridge` a `DataSecuritySettingsView` en `MacSettingsView` y `SettingsWorkspaceView`, solucionando el crash por falta de inyección de entorno en la vista de zona de peligro.

### Added

- Borrado modular por categorías (`SelectiveWipeSheet.swift`): Nueva vista modal que permite al docente seleccionar exactamente qué información eliminar (Cursos y evaluaciones, Alumnado, Rúbricas, Situaciones de aprendizaje, Horario y planificador, Reuniones, Auditoría de IA) con presets de selección rápida ("Solo Cursos y Notas", "Todo menos Rúbricas y SA", etc.). Incorpora un interruptor para realizar una copia de seguridad automática completa (`.sqlite`) inmediatamente antes de vaciar las tablas seleccionadas, protegiendo explícitamente el historial de copias de seguridad de la app. Incluye rediseño adaptativo Apple HIG (680×740 pt en macOS, detents en iOS), tarjetas con estado de preservación/borrado y relanzamiento automático de la aplicación en macOS mediante `NSWorkspace.shared.openApplication` y `NSApp.terminate`.

- Ajustes: nueva sección "Gestión de datos" (`DataManagementSettingsView`), separada de "Datos y seguridad" (que sigue siendo solo el borrado nuclear de toda la app). Permite borrar rápido y en lote Cursos, Asignaturas, Rúbricas y Situaciones de aprendizaje: selección múltiple por sección, "Eliminar (N)" con confirmación y aviso específico por tipo de entidad (un curso borra en cascada alumnado/asistencia/evaluaciones/notas; una asignatura solo desvincula los cursos que la usan), y borrado individual por swipe. El borrado en lote de cursos/asignaturas/situaciones captura el error de cada elemento por separado (a diferencia del patrón existente en `LearningSituationsWorkspaceView`, que corta el bucle entero al primer fallo). El de rúbricas verifica el resultado real refrescando `bridge.rubrics` tras el borrado, porque `KmpBridge.deleteRubric(id:)` no es `async throws` — dispara un borrado en segundo plano en Kotlin (`RubricsViewModel.deleteRubric`) que traga cualquier error internamente; ese bug queda fuera de alcance de esta tarea de UI (vive en `kmp/shared/viewmodel/`). No se modifica `KmpBridge.swift` (archivo protegido): por eso el aviso de borrado de un curso es un texto fijo sobre qué se pierde en cascada, no una cifra exacta de alumnos matriculados (habría requerido un método nuevo en el bridge).
...
- Importación de horario (Excel): el sheet de previsualización del asistente (`ScheduleImportPreviewSheet`, paso "Horario" → Importar) gana una tarjeta "Se creará" entre el resumen y el control de huecos vacíos: lista los grupos y asignaturas nuevos, cuántos se reutilizan del catálogo, y un interruptor único ("Añadir las asignaturas a mi catálogo") para desactivar la creación de asignaturas sin afectar a los grupos (que nunca son opcionales: sin grupo no hay franja que guardar). Los avisos del plan (asignatura sin nombre completo, grupo con varias materias sin `subjectId` fijo) se muestran junto al resto de avisos de la tarjeta.
...
- Context engineering para agentes (Claude 5 / Codex): `.claude/skills` era un directorio inexistente, así que Claude Code no cargaba ninguna de las 23 skills de `.agents/skills/` (solo quedaban referenciadas como texto en `AGENTS.md`) — riesgo de paridad ya señalado en el issue #121. Se añade `.claude/skills` como symlink a `.agents/skills/` para que ambos agentes lean la misma fuente. De paso, se elimina duplicación de contenido entre `CLAUDE.md`, `AGENTS.md`, `docs/AGENT_WORKFLOW.md` y la skill `registrar-avance-app` (reglas de commits/changelog/PR y lista de archivos protegidos, que vivían repetidas en 2-3 sitios): ahora cada regla vive en un único archivo y el resto remite a él. `AGENTS.md` pasa de 225 a ~165 líneas quitando reglas rígidas de alcance ("1-3 archivos") y relleno genérico de UI/UX, y `jobs-design-philosophy` se divide en `SKILL.md` (principios, cargado siempre) y `proceso-auditoria.md` (fases y checklist de auditoría completa, cargado solo cuando la tarea lo pide).

### Changed

- Ajustes (macOS): "Borrar todos los datos" borraba de inmediato el archivo SQLite del disco, pero el proceso en ejecución seguía sirviendo el estado en memoria (conexión SQLite anterior, `KmpBridge` con sus `@Published`) hasta reiniciar manualmente la app — el usuario lo veía como "no se borra nada". Tras completar el borrado, macOS relanza automáticamente la aplicación (`NSWorkspace.openApplication` con una nueva instancia + `NSApp.terminate`, con un pequeño retardo para que se vea el aviso antes del relanzamiento); `RestartRequiredOverlay` cambia su copy en macOS a "Reiniciando…" con `ProgressView`. **Limitación no evitable**: iOS/iPadOS no permite que una app se autorelance (lo rechaza App Review), así que ahí se mantiene sin cambios el aviso de reinicio manual.
...
- Importación de horario (Excel): al confirmar la importación desde el asistente ("Configurar mi horario" → paso Horario → Importar), ahora se crean automáticamente en el catálogo tanto los grupos (ya lo hacía) como las **asignaturas** que detecta el Excel — hasta ahora el catálogo de `Subject` nunca se tocaba y el nombre de la asignatura quedaba solo como texto libre en la franja. Cuando un grupo imparte una única asignatura (excluyendo tutoría), se le asigna automáticamente esa asignatura (`subjectId`), incluido el backfill en grupos ya existentes sin asignatura fija; si imparte varias, se deja sin asignatura fija y se avisa en la previsualización. La reimportación del mismo Excel deja de estar bloqueada por conflicto: las franjas idénticas (mismo grupo, día, hora y materia) se reclasifican como aviso y se omiten al guardar en vez de bloquear el botón "Importar horario".
- Importación de horario (Excel): nuevo `ScheduleImportCatalogResolver` (`kmp/iosApp/AppleShared/ScheduleImportCatalogResolver.swift`) que resuelve, contra el catálogo actual de Cursos y Asignaturas, qué grupos/asignaturas detectados en el Excel ya existen y cuáles hay que crear. Emparejamiento de asignaturas en cascada (código normalizado → nombre de la leyenda del Excel contra el nombre de una asignatura existente → crear), sin sobrescribir nunca el nombre del catálogo. Emparejamiento de grupos por nombre normalizado vía `ScheduleGroupNaming.displayName(forCode:)` en vez de por el patrón "ESO" que usaba `groupCode(from:)`: un grupo de Bachillerato o FP ya se reconoce como existente en reimportaciones en lugar de duplicarse. Todavía no está conectado al flujo de importación (siguiente entrada de este bloque).
...
- Ajustes: "Borrar todos los datos" no tocaba la sincronización, así que el borrado podía **deshacerse solo**. El dispositivo seguía emparejado y con su cursor (`sync.last.cursor`) intacto, de modo que el primer pull tras reiniciar volvía a traerse todos los datos desde el iPad emparejado: la docente confirmaba "BORRAR", reiniciaba y se encontraba sus grupos y notas de vuelta. Además, en macOS el helper de Sync LAN corre en un **proceso aparte** con su propia conexión SQLite al mismo fichero y seguía sirviendo `/sync/pull` durante y después del borrado (riesgo que el fix anterior dejó registrado explícitamente como no resuelto). Ahora `performWipe()` desempareja con `bridge.unpairLanSync()` — que avisa al servidor, cancela las tareas de sync, para el listener SSE y borra token, host, fingerprint y cursor — y a continuación, solo en macOS, publica `.appleCommandCenterStopRequested` para detener el helper antes de vaciar la base. El orden importa: primero desemparejar (necesita la red y el helper vivo), después pararlo. El borrado es **local a este dispositivo**: no se generan tombstones ni se propaga la eliminación al iPad, deliberadamente, para que un borrado accidental en el Mac no vacíe también el otro dispositivo. El diálogo de confirmación lo advierte ("También se desvinculará la sincronización con otros dispositivos").

### Verification

- `xcodebuild -scheme MiGestorKMPMac -configuration Debug -destination 'platform=macOS,arch=arm64'`: `BUILD SUCCEEDED`.

- Ajustes: "Borrar todos los datos" dejaba de borrar el fichero SQLite en caliente, que era la causa raíz del crash que ya se había parcheado dos veces por síntomas (`stopBackgroundSyncWork()` y los guards de `needsRestart`). `wipeAllData()` (`SettingsDangerZoneView.swift`) eliminaba `desktop_mi_gestor_kmp.db`, `-wal` y `-shm` del disco mientras el driver de SQLDelight los tenía abiertos; macOS invalidaba entonces todos los descriptores del pool de conexiones (`BUG IN CLIENT OF libsqlite3.dylib: vnode unlinked while in use` + `invalidated open fd`), la siguiente consulta fallaba con `SQLITE_IOERR` y el proceso abortaba. Mientras el borrado siguiera destruyendo el fichero bajo el driver, cada ruta nueva que tocara la base tras borrar era un crash en potencia, y había que ir tapándolas de una en una. Ahora la base se **vacía por SQL** con la conexión que ya está abierta: nuevo `wipeAllUserData()` (`kmp/data/src/commonMain/kotlin/com/migestor/data/platform/DatabaseWipe.kt`) enumera las tablas desde `sqlite_master` (en vez de una lista fija, para que una tabla nueva no se quede sin borrar en silencio), las vacía dentro de una única transacción con `PRAGMA defer_foreign_keys = ON` (el orden de borrado deja de importar), reinicia `sqlite_sequence` y hace `VACUUM` fuera de la transacción. No se invalida ningún descriptor, no puede fallar por E/S y el esquema y `user_version` quedan intactos. Se expone como `KmpContainer.wipeAllData()` y, por necesidad, como `KmpBridge.wipeAllDatabaseData()` (archivo protegido: el contenedor es privado y la vista no puede alcanzarlo sin abrir un driver nuevo por su cuenta). Los ficheros que nadie tiene abiertos (copias de seguridad, adjuntos) se siguen borrando desde Swift, y ahora un fallo al borrarlos se comunica en pantalla en vez de tragarse con `try?`. El aviso de reinicio se mantiene: aunque la base ya no se rompa, los cachés en memoria y los `Flow` ya suscritos conservan los datos viejos (el vaciado no pasa por las queries generadas de SQLDelight, así que no notifica a sus listeners).

### Verification

- `:data:desktopTest --tests "com.migestor.data.platform.DatabaseWipeTest"`: 3/3 en verde. Nuevo `DatabaseWipeTest` que verifica que el borrado deja todas las tablas de usuario a 0, que conserva el conjunto de tablas y `user_version`, que **el mismo driver sigue usable después** (una escritura posterior funciona — con el borrado por fichero fallaba con `SQLITE_IOERR`) y que es idempotente sobre una base ya vacía.
- `xcodebuild -scheme MiGestorKMPMac -configuration Debug -destination 'platform=macOS,arch=arm64'`: `BUILD SUCCEEDED`.

- KMP ↔ Swift: un fallo de base de datos en cualquier `suspend fun` de un repositorio abortaba el proceso entero en vez de propagarse a Swift como error capturable. De las 232 `suspend fun` declaradas en `kmp/shared/src/commonMain/kotlin/com/migestor/shared/repository/Contracts.kt`, solo 24 estaban anotadas `@Throws(Exception::class)`. Kotlin/Native exporta toda `suspend fun` a Swift como `async throws`, pero en tiempo de ejecución solo convierte a `NSError` las excepciones declaradas en `@Throws`; cualquier otra se considera "unexpected and unhandled" y termina el proceso (`Kotlin_ObjCExport_runCompletionFailure` → `pthread_kill` → SIGABRT). Esa es la causa directa de que un `SQLITE_IOERR` en `DashboardRepository.getStats()` (una de las 208 sin anotar) tumbara la app en vez de mostrar un error. Se anotan las 208 restantes: el cambio es puramente aditivo (208 líneas nuevas, ninguna modificada) y no altera comportamiento en JVM/desktop, donde `@Throws` es inerte. Los overrides concretos de `kmp/data` heredan la lista sin necesidad de repetir la anotación (verificado compilando `:shared:compileKotlinMacosArm64` y `:data:compileKotlinMacosArm64` sin errores de `INCOMPATIBLE_THROWS_OVERRIDE`).

- Ajustes (macOS): "Borrar todos los datos" seguía pudiendo hacer abortar la app incluso tras el fix anterior (`stopBackgroundSyncWork()`), si el usuario cerraba la ventana en vez de esperar al reinicio automático — crash real reproducido y reportado por el usuario. Cerrar la ventana justo después del borrado dispara la transición de `scenePhase` a `.background` (`MiGestorKMPMacApp` → `MacAppSessionController.handleScenePhase` → `KmpBridge.onAppDidEnterBackground()`), que lanza un `Task` **nuevo** llamando a `syncNow(reason: "background_flush", ...)` — un `Task` que no existía todavía cuando `wipeAllData()` llamó a `stopBackgroundSyncWork()`, así que no había nada que cancelar. Ese `syncNow` toca repositorios Kotlin no declarados `@Throws` sobre el fichero de base de datos que el borrado acababa de eliminar, mismo mecanismo de crash que el fix anterior. `syncNow(reason:forceFullPull:silent:)` gana un guard centralizado (`!AppleBackupService.shared.needsRestart`) al principio, que protege de una vez a todos sus puntos de entrada (arranque, primer plano, segundo plano, bucle de auto-sync, listener SSE, helper listo); se añade el mismo guard en `onAppDidEnterBackground()` (evita lanzar el `Task` siquiera) y en `checkLocalDbFileModification()` y la iteración del bucle de auto-sync (que hacían su propia ráfaga de `refresh*()` sin pasar por `syncNow`).
- Ajustes: "Borrar todos los datos" parecía no borrar nada (reportado por el usuario tras confirmar el borrado con "BORRAR" y comprobar que, al reabrir la app, seguían apareciendo el grupo "3 ESO A" con Ana López y Pablo García y sus notas). El borrado en sí funcionaba: lo que ocurría es que `KmpContainer.seedDemoDataIfEmpty()` (`kmp/data/src/commonMain/kotlin/com/migestor/data/di/KmpContainer.kt`) se ejecuta en cada arranque de la app (`KmpBridge.bootstrap()` → `seedIfNeeded()`) y siembra un grupo y alumnado de demostración en cuanto detecta que no hay ningún alumno — pensado para que la primera instalación no se vea vacía, pero disparándose también después de un borrado deliberado y deshaciéndolo. `bootstrap()` ahora guarda una marca en `UserDefaults` (`demo.seed.completed.v1`, deliberadamente fuera del SQLite que `wipeAllData()` borra) la primera vez que siembra, y no vuelve a sembrar nunca más aunque la base de datos quede vacía tras un borrado.
- Ajustes: "Borrar todos los datos" podía hacer abortar la app (crash real reportado por el usuario, confirmado en Xcode con `Kotlin_ObjCExport_runCompletionFailure` → `pthread_kill`). `wipeAllData()` (`SettingsDangerZoneView.swift`) borra el SQLite del disco mientras el proceso sigue con la conexión abierta, pero nunca detenía el trabajo en segundo plano que la sigue tocando: el bucle de auto-sync (arranca solo en cuanto el helper de Sync LAN local está listo, sin necesidad de haber emparejado nada), los debounces de guardado de notas/cuaderno/calificaciones (700–900ms) y el listener de eventos de sync. Ninguna de esas llamadas a los repositorios Kotlin está declarada `@Throws`, así que si intentan tocar un fichero que acaba de desaparecer, el runtime de Kotlin/Native trata la excepción como fatal en vez de propagarla como error de Swift capturable. Nuevo `KmpBridge.stopBackgroundSyncWork()` (cancela las 7 tareas y para el listener SSE) se llama al principio de `wipeAllData()`, antes de borrar ningún fichero. **Riesgo relacionado, no resuelto en esta tarea**: en macOS el helper de Sync LAN corre en un proceso aparte con su propia conexión SQLite al mismo fichero; este fix no lo detiene, así que ese proceso podría fallar de forma independiente si toca la base de datos en el mismo instante del borrado (impacto menor: es un subproceso, no tumba la app principal).
- Ajustes (macOS): al entrar en "Zona de Riesgo" (Ajustes → Datos y Seguridad → Acciones destructivas), el resto de la navegación de la app se quedaba bloqueada mostrando esa pantalla aunque el usuario cambiara de sección en la barra lateral principal. `MacSettingsView.swift` renderizaba su detalle en un `HStack`/`ScrollView` sin ningún `NavigationStack`, así que el `NavigationLink` de `DataSecuritySettingsView` (compartido con iOS, donde sí hay `NavigationStack`/`NavigationSplitView`) resolvía el push contra un contexto de navegación implícito ligado a la ventana en vez de al árbol de vistas de Ajustes — al recrear el resto de la app con otra `feature`, ese push huérfano sobrevivía y seguía tapando el panel de detalle. Se envuelve el contenido de detalle de `MacSettingsView` en su propio `NavigationStack` con `.id(selectedRoute)`, para que el push quede ligado a esa identidad y se destruya limpiamente al cambiar de sección (dentro o fuera de Ajustes).
- Importación de horario (Excel): en una celda con varios grupos separados por `/` (p.ej. `EFI:1ESOA / MUS:2ESOB`), `ScheduleExcelImportService.parseCell` acumulaba los códigos de asignatura y de grupo en dos arrays independientes que se deduplicaban por separado, perdiendo el emparejamiento real de la celda. `ImportedScheduleSlot` gana `assignments: [ImportedSlotAssignment]` (pares asignatura↔grupo) y `subjectCode(forGroup:)`; `subjectCodes`/`groupCodes` pasan a ser propiedades computadas derivadas, sin cambios de comportamiento para el resto de la app. Sienta la base para que el guardado de franjas escriba la asignatura correcta por grupo (siguiente entrada de este bloque).
- Situaciones de aprendizaje: al seleccionar otra situación en el maestro, "Documento fuente" y "Vínculos creados" seguían mostrando la versión y los recursos de la situación anterior. `selectedSituationId` no disparaba `reloadDetail()` (solo se llamaba desde `reload()`, import, duplicar o borrar), así que la selección cambiaba en la lista pero el panel de detalle no. Se añade `.appOnChange(of: selectedSituationId)` para recargar versiones, vínculos y grupos al cambiar de selección.
...
- Planner (Secuencia/Gantt): las sesiones **canceladas** se pintaban con el mismo color de acento que las planificadas (indistinguibles de un vistazo) porque `PlannerGanttSessionMark.tint` y `segmentColor(for:)` re-derivaban el color por su cuenta comparando texto (`localizedCaseInsensitiveContains("cerrad"/"impart")` en un sitio, comparación exacta de `statusText` en otro) en vez de usar `PlannerSequenceRow.statusColor`, que el ViewModel ya calculaba correctamente pero que la vista ignoraba por completo. Además ese `statusColor` usaba `Color.green/.yellow/.red/.orange` fijos, fuera del ajuste de contraste en modo oscuro que la Fase 8 del plan premium ya había aplicado a `EvaluationDesign.success/.accent/.danger`. Se unifica todo a una única fuente de color (`row.statusColor`, ahora con los tokens de `EvaluationDesign`/`IOSAppStyle`) y se añade "Cancelada" a la leyenda del Gantt, antes invisible como estado propio.

### Added

- Planner (Secuencia/Gantt): el chip "Vas N sesión(es) por detrás" de cada grupo pasa a ser un botón cuando el grupo va con retraso — filtra por ese grupo y salta a la Semana para que el docente pueda actuar directamente, en vez de solo informar del problema. Se añade también un aviso de "cómo leer esto" (fila = situación, marca = sesión) que se muestra una sola vez (`@AppStorage`, mismo patrón que el aviso de arrastre del grid semanal) y un contador "Quedan N semanas lectivas" en el panel de Cobertura del Resumen, calculado contra `vm.scheduleEndDate`, para dar contexto al porcentaje de cobertura.
- Dashboard (macOS): Resumen por grupo, Agenda docente, Educación Física, Auditoría LOMLOE y la fila de KPIs, hasta ahora exclusivos de iPad, se renderizan también en macOS bajo "Ahora"/Pendientes/Riesgo. Antes macOS solo tenía un modelo ad-hoc (`MacDashboardSnapshot`) derivado del horario fijo del profesor, sin ninguna vía al `DashboardSnapshot` real que ya calcula el backend (`GetOperationalDashboardSnapshotUseCase`); ahora `loadDashboard()` también llama a `bridge.refreshDashboard(mode: .office)` y los nuevos bloques se pintan sobre ese snapshot. "Ahora" (franja horaria activa/próxima) y Pendientes/Riesgo/Sistema, que sí dependen del horario fijo y del almacén de backups —sin equivalente al otro lado—, se quedan como estaban a propósito. Los bloques en sí viven ahora en `DashboardSharedBlocks.swift`, compartido con iPad en vez de duplicado.
- Situaciones de aprendizaje: tres datos que el backend ya guardaba y la interfaz no mostraba nunca. (1) El estado (`LearningSituationStatus`: Borrador/Activa/Archivada) se persiste y sincroniza, pero no aparecía en la lista ni en el detalle ni se podía cambiar; ahora hay un badge en ambos sitios y un submenú "Estado" para cambiarlo (`KmpBridge.updateLearningSituationStatus`, nuevo). (2) Los avisos del import (`LearningSituationVersion.warningsJson`) solo se veían en el momento de importar el documento; ahora "Documento fuente" los muestra también después, junto al sha256 y el tamaño. (3) "Vínculos creados" pasa de una lista de `Label` inertes a filas navegables: cada recurso (sesión de planificación, evaluación, rúbrica, columna de cuaderno) lleva al módulo correspondiente con `onOpenModule`, cerrando el círculo importar → programar → evaluar que antes se cortaba en el último paso.
- Dashboard (iPad) y Situaciones de aprendizaje: estados vacíos con una salida de verdad, en vez de solo texto. Con cero clases, el Dashboard dejaba de mostrar un dashboard y pasaba a ser diez tarjetas diciendo "Sin sesiones hoy"/"Sin datos de grupos"/"Sin agenda para hoy" a la vez — ahora ese caso muestra un único estado (`dashboardEmptyState`) con "Crear clase" y accesos a Cursos, Planner y Situaciones, igual que ya tenía macOS (`DashboardEmptyStateView`). En Situaciones, el estado vacío pasa de un texto sin botón (la única entrada era el "+" de la cabecera) a incluir "Importar situación", que abre el mismo selector de documento. De paso, `PremiumEmptyState`/`WorkspaceEmptyState` ganan un `actionTitle`/`action` opcionales y retrocompatibles, reutilizables por cualquier otra pantalla con estado vacío.
- Dashboard (iPad): jerarquía visual en dos niveles en vez de diez tarjetas idénticas. La fila de KPIs (Hoy/Alertas/Pendientes/Próxima sesión) pasa a ser el resumen numérico único arriba del todo, en vez de repetir después los mismos contadores que ya llevaban las cabeceras de "Hoy" y "Pendiente" (se elimina esa duplicación). Los filtros de severidad/prioridad/sesión se mueven justo encima de Hoy/Pendiente/Riesgo, las tres secciones a las que afectan, en vez de aparecer varias secciones después. Resumen por grupo, Agenda docente, Educación Física, Sistema y Auditoría LOMLOE —contexto, no acción— pasan a un cromado plano (`.thinMaterial`, sin sombra de color, título más pequeño) para distinguirse de Hoy/Pendiente/Riesgo/Evaluación rápida, que mantienen el tratamiento con sombra por ser las tarjetas de "hay que actuar".
- Cuaderno: el peso y la exclusión de la media pasan a verse en la propia rejilla, en vez de solo dentro de "Configurar media". La cabecera de cada columna añade `× N` cuando su peso no es el estándar (×1) o `no cuenta` cuando está excluida de la media (`columnHeaderMeta`/`columnWeightBadge`), así que una columna excluida deja de ser indistinguible de una que pesa ×3 con solo mirar la rejilla. La cabecera de "Media" deja de parecer una columna de sistema más: gana un icono de ajustes y un subtítulo dinámico ("Ponderada · N col.") que recuerda cuántas columnas cuentan hoy y que tocar ahí abre su configuración.
- Dashboard (iPad): las alertas de riesgo/pendientes, la agenda docente y los bloques de Educación Física ahora navegan a un módulo concreto en lugar de abrir solo un panel de detalle de solo lectura. El backend (`GetOperationalDashboardSnapshotUseCase` → `AgendaItem.navigationTargets`) ya resolvía, por alumno, qué rúbrica en concreto le falta por evaluar, pero esa información nunca llegaba a la interfaz de iPad: el inspector de alerta/sesión/EF era un panel de texto sin acciones, y la Agenda docente y los pendientes se pintaban con filas inertes. `DashboardView` recibe ahora `onOpenModule` (como el resto de módulos del workspace) y lo usa para: abrir Rúbricas en el alumno y evaluación pendientes cuando la alerta trae `navigationTargets`; ir a la ficha del alumno o al cuaderno cuando solo hay `studentId`/`classId`; y saltar a Asistencia, Planner o el módulo de EF correspondiente desde la Agenda y los bloques EF. Mismo patrón de navegación que ya usaba macOS (`onNavigate`), esta vez cableado al modelo real del backend en vez de a un modelo ad-hoc.
...
- Planner: **configurador de la agenda docente, reubicado y rediseñado como asistente progresivo**. Antes solo era alcanzable cambiando a la pestaña "Resumen" del Planner (no la que se ve por defecto) y pulsando uno de dos botones pequeños — uno de los cuales, además, solo aparecía cuando ya había franjas configuradas, así que el profesor sin agenda (el que más la necesita) no lo veía. En iPad la ruta estaba directamente rota: navegaba a Ajustes, que no tenía ninguna sección de horario, y el panel que debía alojarlo (`TeacherScheduleSettingsPanel`) nunca llegó a conectarse a ningún punto de entrada — código muerto. Ahora: (1) nueva sección **"Horario docente"** en Ajustes en ambas plataformas, el destino canónico; (2) la vista "Semana" del Planner (la que se ve por defecto) muestra un estado vacío accionable con el botón "Configurar mi horario" cuando no hay franjas, en vez de una rejilla en blanco; (3) el botón de "Ajustar agenda" del panel de Cobertura ya no se esconde justo cuando no hay franjas. El propio configurador (`TeacherScheduleWizard`, nuevo) sustituye al scroll monolítico de siete secciones (algunas colapsadas, la mayoría no, y en iPad ninguna) por un asistente de tres pasos — Curso → Horario → Terminado —, una tarea por pantalla; Evaluaciones, Unidades didácticas, Colores y No lectivos pasan a refinamientos opcionales accesibles desde el paso final, nunca forzados en el camino principal. Sustituye a `MacTeacherScheduleSettingsPanel` y `TeacherScheduleSettingsPanel` (duplicados entre sí, ~1700 líneas eliminadas). Ubicación consultada con un agente Opus dedicado antes de implementar.
- Planner: en el paso "Curso" del asistente de horario, el rango de fechas pasa de dos `DatePicker` sueltos compitiendo en peso visual con el campo de nombre a ser la única tarjeta con peso fuerte (cifra de curso grande + línea de tiempo sept→jun) según el test del bizqueo, con dos presets de un toque ("curso actual"/"curso siguiente", ambos 1 sept – 30 jun calculados desde la fecha de hoy) para el caso común; los selectores finos, los días lectivos y el nombre de agenda bajan de rango a ajuste secundario. El nombre por defecto ya salía bien calculado en el backend (`ensureAcademicYear`/`fallbackSchedule` ya usaban 1 sept–30 jun del año en curso); lo que faltaba era ese mismo rango accesible en un toque en vez de tecleado en dos `DatePicker`.
