...
- Planificacion: sesiones, situaciones de aprendizaje y continuidad docente.
  ...
  Avance: Planificación macOS gana paridad de diario de sesión con iPadOS (sheet flotante que reutiliza `DiaryWorkspaceView`) y su detalle de sesión se presenta como sheet propio de ancho completo en vez de un inspector genérico estrecho. La vista Día suma una etiqueta derivada "Confirmar impartida" para franjas ya pasadas (sin escribir estado en BD en automático) y un sheet de "Diario rápido" para cerrar el diario de todas las sesiones del día con el mínimo de gestos.
