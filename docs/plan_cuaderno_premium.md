...
### 2.1 Hover de fila sin invalidar los 3 paneles (macOS) — hecho (alcance corregido)
- Fix aplicado: `NotebookGridContainer` sustituye el `@State hoveredRowId` compartido por `@StateObject private var hoverModel = NotebookRowHoverModel()` (un `ObservableObject` con un único `@Published var hoveredRowId: AnyHashable?`). La lógica de fondo/overlay/`.onHover` se movió a una vista hoja nueva, `NotebookHoverableRow`, que observa `hoverModel` con `@ObservedObject` y calcula `isHovered` comparando su propio `rowId`.
...
- **Corrección respecto a la primera versión de este documento**: `@StateObject`/`@ObservedObject` invalida la vista que los declara ante cualquier publicación del objeto, con granularidad por objeto, no por propiedad — igual que ocurría con el `@State` original. Es decir, `NotebookGridContainer.body` probablemente se sigue re-evaluando en cada transición de hover, igual que antes; el `@StateObject` no es una forma de "esconder" ese cambio de su propio dueño. El beneficio real y verificable por lectura de código es más modesto que lo escrito originalmente: la comparación `isHovered` y el fondo/overlay ya no viven inline en el cierre de `rowStack`, sino en un tipo con nombre propio (`NotebookHoverableRow`) más fácil de aislar/optimizar después (por ejemplo, añadiendo `Equatable` explícito si Instruments confirma que hace falta), y el hover deja de depender de una transacción `withAnimation` global. No se puede afirmar sin medir que el coste por transición de hover haya bajado de forma sustancial.
- Archivo: `NotebookGridContainer.swift`. Verificado con `swiftc -parse`; **medición real en Instruments pendiente** (requiere Xcode.app/dispositivo, no disponible en este entorno) para confirmar si hace falta ir más allá (p. ej. mover la comparación de hover a nivel de gesto de AppKit por fila, fuera de SwiftUI, si el coste medido lo justifica).
...
### 2.4 Trocear `NotebookModuleView` — en progreso (2 de 5 stores)
- Patrón usado (igual que `NotebookMacInspectorState` ya existente): una clase `ObservableObject` nueva con los `@Published` correspondientes, un `@StateObject` en `NotebookModuleView` apuntando a ella, y propiedades computadas `get`/`set` en `NotebookModuleView` con el **mismo nombre** que el `@State` original (`undoStack`, `todayAttendanceByStudentId`, etc.), para que `NotebookModuleSupportActions.swift`/`NotebookModuleToolbarState.swift`/`NotebookModuleDisplayFormatting.swift` seguían compilando sin tocarlos. Cambio puramente estructural, sin alterar comportamiento.
- Hecho:
  - `NotebookUndoStore` (`undoStack`, `redoStack`) — reemplaza los `@State` sueltos añadidos en 1.1.
  - `NotebookAttendanceSignalsStore` (`todayAttendanceByStudentId`, `incidentCountByStudentId`) — este último se lee también desde `NotebookModuleDisplayFormatting.swift:attendanceSummary(for:)`, usado en el plano de aula.
  - Ambas clases viven en `NotebookModuleTypes.swift`, junto a `NotebookMacInspectorState`.
- Pendiente (mismo patrón, cada uno un incremento propio): `riskLevelCache`/`riskComputationKey`/`isPrecomputingRiskLevels`, `rowReloadRevisions`, `seatPositions`.
- Corrección sobre el diagnóstico de 2.1: revisando esto con más detalle, `@StateObject`/`@ObservedObject` invalida la vista que los declara ante **cualquier** cambio publicado del objeto (granularidad por objeto, no por propiedad) — igual que `@State`. El beneficio real de este troceo no es "evitar que `NotebookModuleView.body` se re-evalúe nunca", sino (a) reducir el número de `@State` que cualquier edición de celda toca a la vez dentro de una única vista gigante, y (b) permitir que subvistas que reciban un store concreto como `@ObservedObject` puedan reaccionar sin depender de que el ancestro entero también lo intente. Es una mejora de organización y de cota de invalidación razonable y de bajo riesgo, no una garantía de "cero re-render" verificable sin Instruments.
- Aceptación: cada store compila (`swiftc -parse`) sin tocar los ficheros consumidores; comportamiento idéntico verificado por lectura de código (mismos nombres, mismas operaciones).
