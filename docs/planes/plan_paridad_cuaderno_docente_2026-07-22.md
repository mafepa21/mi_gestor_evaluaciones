...
- **D-3 — CORREGIDO.** Las filas del heatmap eran ventanas móviles de 7 días contadas desde hoy, no semanas lectivas: `weeksDifference` salía de `dateComponents([.weekOfYear], from: fecha, to: Date())`. Comprobado con datos: incidencias del **viernes 17-jul** y del **lunes 20-jul** —semanas escolares distintas— caían ambas en la fila `S-3`. Ahora cada fila es una semana natural de lunes a domingo y se etiqueta con la fecha de su lunes (`6/7`, `13/7`, `20/7`), en orden cronológico descendente en pantalla. Verificado en la app: los mismos dos registros caen ya en filas distintas (`13/7 · V` y `20/7 · L`).
- **D-2 — CORREGIDO.** El bucle iteraba `for weekday in 2...8` frente a los valores reales 1…7 de `Calendar.component(.weekday)`: la columna "D" no podía recibir nada y el domingo no se contaba. Verificado en la app con un registro en domingo 19-jul, que ahora aparece en `13/7 · D`.
- **Nota de método.** La primera versión de la corrección usaba `calendar.dateInterval(of: .weekOfYear, for:)` con un fallback a `startOfDay`. Se descartó: depende de `firstWeekday` y de la configuración regional, y si el fallback se activa convierte las filas en días sueltos y deja la rejilla entera a cero **sin ningún aviso**. La versión final calcula el lunes con aritmética explícita sobre el día de la semana.
