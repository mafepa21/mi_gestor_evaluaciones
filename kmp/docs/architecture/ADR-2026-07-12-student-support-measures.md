...
- **`measure_type` es un enum cerrado**, no texto libre, para que el badge del Cuaderno y los filtros sean fiables: Nivel III (`REFUERZO`, `ENRIQUECIMIENTO`, `ADAPTACION_ACCESO`, `APOYO_PT`, `APOYO_AL`) y Nivel IV (`ACIS`, `EXENCION`, `FLEXIBILIZACION`, `PERMANENCIA_EXTRAORDINARIA`, `ESCOLARIZACION_ESPECIFICA`).
...
- Si se amplía `measure_type` con nuevas medidas de la normativa, mantener el enum cerrado en KMP (`kmp/shared/domain/Models.kt`) en vez de admitir texto libre.
